...
**Notification**:
The persistent in-app record of something that concerns one User. Lives in the Notification feed and, if a tab is open, is mirrored as a browser notification. Three things produce one: a Reminder on the Notification Channel firing (where the Reminder is the rule and the Notification the delivered instance), the User being made an Attendee of an Event, and a Write-back push permanently failing on a writable Linked Calendar — the last coalesced on the Calendar rather than the Event, since a dead Connection fails every one of its queued pushes at once and would otherwise raise dozens. Deliberately nothing else beyond these three — being removed as an Attendee, an Event moving, and another Attendee's Response all produce none, the last because the Attendee list already shows every Response and a fourteen-person invite would otherwise mean fourteen entries. The Organizer never gets one for their own Event, since an Organizer is never an Attendee. Distinct from a toast, which is a transient client-only UI message (e.g. an error). See ADR-0021, ADR-0061, ADR-0081.
...
**Exposure**:
One User's own answer to whether a Calendar appears in *their* CalDAV home-set — resolved per User rather than stored per Calendar, so the same Calendar can sit in one accessor's home-set while staying absent from its own Owner's, or from another accessor's. Every Calendar accepts an explicit choice in either direction; absent one, the default is present, except a Linked Calendar's own Owner, who defaults to absent. Resolved only after Access, never in its place — a Calendar the User can no longer read is never exposed regardless of what this holds, and a Share ending clears it the same way it clears a colour override. See ADR-0080.
_Avoid_: visibility, sharing, publishing, device sync
...
**Linked Calendar**:
A Calendar mirrored from one calendar of a Connection. An ordinary Calendar in every respect a User can see, distinguished by what stands behind it: a Connection the connecting User authorized, not a URL anyone can paste. Owned by that User even when the underlying calendar was merely shared to them at the Provider — Access resolves normally and is then clamped by the Source's mode, so a calendar the Provider will not accept writes to is read-only here for its Owner too. Shareable to Workspace Members like any other Calendar, but a Share on it is clamped to Viewer: an Editor's write would run as the connecting User's own Provider identity, so the Editor Role is refused at creation, not silently downgraded. Its CalDAV home-set membership is governed by Exposure (ADR-0080): its own Owner defaults to absent, since the Owner almost certainly syncs that calendar natively at the Provider already, while a Member it was Shared to defaults to present, having nothing of their own to duplicate — either default is overridable per User. What it looks like is this instance's business, not the Provider's: its Name follows the Provider's own renames until someone renames it here, reusing a Subscription's shadow-column mechanism verbatim (ADR-0032); its Calendar color is seeded from the Provider once at import and is this instance's afterward, with no publisher value left to track beneath it. See ADR-0052, ADR-0075, ADR-0080.
...
**Release**:
A published, immutable set of artifacts cut from a single named point in history, and the act of publishing it. One Release comprises the tag naming it, the container image carrying that name, and the pre-compiled binaries attached alongside — all built from the same source at the same moment, so every artifact in a Release reports the same Version. Cutting one is deliberate and separate from creating the tag: a tag can exist unpublished, and nothing reaches a registry until the Release is published. Distinct from the Version, which is only the label a Release stamps onto the binaries it contains, and distinct from a pre-release, which is a Release that is published and installable but explicitly not what a new self-hoster gets by default. See ADR-0073.
_Avoid_: build, deploy, version (as a synonym), ship

**Version**:
The opaque label naming which build of the app an instance is running, fixed when that build is produced and unchangeable afterward — a property of the code, not of the deployment, so unlike the Data directory it is not something a self-hoster sets. Never parsed, compared, or reformatted: whoever names the Release decides what it reads, and it is shown and served exactly as written. Reads `dev` on any build produced without one, which is every build made outside a Release. Shown beside the wordmark to whoever is signed in, and answerable to anyone who can reach the instance, signed in or not. Distinct from the Release it names — the Release is the published set of artifacts, the Version is only the label they carry. See ADR-0072.
_Avoid_: build number, Release (a different term), app version, semver
