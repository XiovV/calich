...
func newUserDirectoryTestServer(t *testing.T) userDirectoryTestServer {
	...
	auth := service.NewAuthService(users, sessions, service.NewWorkspaceService(sqlDB, repository.NewWorkspaceRepository(sqlDB)), []byte("test-secret"), "caller", "hunter2", false)
