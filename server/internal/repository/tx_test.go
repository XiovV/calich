...
func TestWithTx_CommitsOnSuccess(t *testing.T) {
	...
	err := WithTx(ctx, sqlDB, func(tx *sql.Tx) error {
		_, err := repo.WithTx(tx).Create(ctx, "evt-1", userID, calendarID, "Standup", start, end, false, "", nil, nil, nil, "", "", 0)
		return err
	})
	if err != nil {
...
func TestWithTx_RollsBackOnError(t *testing.T) {
	...
	err := WithTx(ctx, sqlDB, func(tx *sql.Tx) error {
		if _, err := repo.WithTx(tx).Create(ctx, "evt-1", userID, calendarID, "Standup", start, end, false, "", nil, nil, nil, "", "", 0); err != nil {
			return err
		}
		return sentinel
	})
