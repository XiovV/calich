...
import type { CalendarDisposition, CalendarDispositionChoice, RemoveMemberImpact } from "../lib/workspaceMembersApi";
...
// The Member-removal flow (#160, #165): every Calendar the Member being
// removed owns within this Workspace needs its own explicit
// transfer-or-delete choice before the removal completes — there is no
// default, so a shared calendar is never destroyed or handed off because a
// dialog had one pre-selected. Mirrors DeleteAccountDialog's self-Delete
// flow, scoped to one Member of one Workspace instead of the caller's own
// account across every Workspace they belong to.
export function RemoveMemberDialog({ member, onClose }: RemoveMemberDialogProps) {
  const fetchImpact = useWorkspaceMembersStore((state) => state.removeImpact);
  const removeMember = useWorkspaceMembersStore((state) => state.removeMember);

  const [impact, setImpact] = useState<RemoveMemberImpact | null>(null);
  ...
  // identical to "they own nothing" and let this destructive action's
  // confirm button enable without ever having shown what it's about to
  // affect.
  ...
  async function handleConfirm() {
    ...
    await run(async () => {
      await removeMember(member.userId, choices);
      onClose();
    });
  }
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed top-1/2 left-1/2 z-50 w-[30rem] max-h-[85vh] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-shell-lg bg-surface p-5 shadow-elevation-3">
          ...
          {!impact && !impactError && <p className="mt-2 text-label-sm text-ink-muted">Loading…</p>}

          {impact && calendars.length === 0 && (
            <p className="mt-2 text-body text-ink-muted">They don't own any calendars in this workspace.</p>
          )}
                      ...
                      <>
                        {c.transferCandidates.length > 0 ? (
                          ...
                          <p className="mt-2 text-label-sm text-danger">
                            There is no other member of this workspace to transfer this calendar to.
                          </p>
                        )}
                      </>
                    )}
