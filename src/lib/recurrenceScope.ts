...
/**
 * "All events": `changes` as edited in the modal, but anchored back onto
 * `master`'s own date — the modal only exposes a time-of-day field (the
 * "day" shown is whichever Occurrence happened to be opened), so applying
 * `changes.start`/`changes.end` verbatim would silently move the master's
 * `DTSTART` and truncate every earlier Occurrence out of the series.
 */
export function applyAllEventsChanges(
  master: Event,
  changes: EventFieldChanges,
): EventFieldChanges {
  return {
    calendarId: changes.calendarId,
    title: changes.title,
    start: withTimeOfDay(master.start, changes.start),
    end: withTimeOfDay(master.end, changes.end),
  };
}

/** `date`'s own day, with `time`'s hours/minutes/seconds. */
